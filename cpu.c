...
{ 
    ...
    for(i=0;i<TAILLEMEMOIRE;i++) //faisable avec memset
    { 
        cpu.memoire[i]=0; 
    } 
    for(i=0;i<16;i++) 
...
{  
    Uint16 opcode = (cpu.memoire[cpu.pc]<<8)+cpu.memoire[cpu.pc+1];

    if(opcode != 0x0000){
        printf("Opcode recupere : %04X (PC = %04X)\n", opcode, cpu.pc);
    }
    return opcode; 
}
...
void interpreterOpcode(Uint16 opcode) 
{
    ...
    case 23:{ 
            ...
            printf("Opcode DXYN détecté : %04X (VX = %X, VY = %X, N = %X)\n", opcode, b3, b2, b1);
